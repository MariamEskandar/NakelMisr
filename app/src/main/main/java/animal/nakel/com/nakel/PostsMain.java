...
//                    Blog_Posts_id=FirebaseDatabase.getInstance().getReference().child("Blog").child(Posts_id1);
      ...
      //  query.orderByKey().equals(Posts_id1);
        /*
        ...
        Users.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.hasChild("Price")){
                    ...
                    String Posts_id=(String)dataSnapshot.child("Post_id").getValue();

                    Blog_Posts_id=FirebaseDatabase.getInstance().getReference().child("Blog").child(Posts_id);
        ...
        */
      //  Query query = mDatabaseMainActivity.orderByChild("PostTime").limitToLast(20);
   //  Query query2 = mDatabaseMainActivity.orderByKey();
     // String CurrentUserId=mAuth.getCurrentUser().getUid();
      //mQueryCurrentUser=mDatabaseCurrentUser.orderByChild("uid").equalTo(CurrentUserId);

        //query_Accept =mDatabaseMainActivity.orderByKey().equalTo(Posts_id1);
        DatabaseReference mm =FirebaseDatabase.getInstance().getReference().child("Blog").child("Comment");
        String ff= mm.toString();
Query kk =mm;
       // query_Accept=mDatabaseMainActivity.child("Comment").child(mCurrentuser.getUid());

       // query_Accept=mm.getKey();
...
*/
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            Intent intent = new Intent(PostsMain.this, PostsMain.class);
            startActivity(intent);
            return true;
        }
